...
- Standalone tabs without a BalanceHeader (home, polls, settings) use `pt-6` top padding to match the BalanceHeader's own `pt-6`. The calendar sub-tab switcher in BudgetApp also uses `pt-6`. Tabs rendered below a BalanceHeader or sub-tab switcher (activity, members, calendar content, events content) keep `pt-4`.
